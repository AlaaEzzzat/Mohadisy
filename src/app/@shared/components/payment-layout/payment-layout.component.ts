...
export class PaymentLayoutComponent implements OnInit {
  ...
  search(word: any) {
    ...
    if (this.userType == 'client' || this.userType == 'sp') {
      ...
      this.data.map((project: any) => {
        if (
          project.profile?.offers[0]?.project?.name.search(
            new RegExp(word, 'i')
          ) != -1
        ) {
          this.dataShow.push(project);
      ...
      this.data.map((project: any) => {
        if (
          project.profile?.projects[0]?.name.search(new RegExp(word, 'i')) != -1
        ) {
          this.dataShow.push(project);
        }
      });
  ...
  sortByName() {
    if (this.userType == 'sp' || this.userType == 'client') {
      ...
      this.data.sort((a: any, b: any) =>
        a.profile.offers[0].project.name.localeCompare(
          b.profile.offers[0].project.name
        )
      );
    } else if (this.userType == 'clientForAdmin') {
      this.data.sort((a: any, b: any) =>
  ...
  sortByDate() {
    if (this.userType == 'sp' || this.userType == 'client') {
      ...
      this.data.map((pro: any) => {
        pro.profile.offers[0].project.dateCreated = new Date(
          pro.profile.offers[0].project.dateCreated
        );
      });
      this.data.sort((a: any, b: any) => {
        return (
          b.profile.offers[0].project.dateCreated -
          a.profile.offers[0].project.dateCreated
        );
      });
    } else if (this.userType == 'clientForAdmin') {
      this.data.map((pro: any) => {
        pro.profile.projects[0].dateCreated = new Date(
          pro.profile.projects[0].dateCreated
        );
      });
      this.data.sort((a: any, b: any) => {
        return (
          b.profile.projects[0].dateCreated - a.profile.projects[0].dateCreated
        );
      });
    }
  }
